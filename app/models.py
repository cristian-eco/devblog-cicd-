...
class BlogPost:
    ...
    def __init__(self, title:str, content:str, author: str  = "Admin"):
        self.id = None #Se asignara automaticamente
    ...
    #Fecha de creacion automatica
        self.updated_at = datetime.now() #Fecha de ultima actualizacion
...
class BlogStorage:
    ...
    #En produccion real se usaria PostgreSQL, MYSQL, etc
    def __init__(self):
        #Inicializa el almacenamiento con algunos posts de ejemplo
        self._posts: List[BlogPost] = []
        self._next_id =1

        #Crear algunos posts de ejemplo para que el blog no este vacio
        self._create_sample_posts()

    def _create_sample_posts(self):
        #Crea posts de ejemplo para demostracion
        sample_posts = [
            {
                'title': 'Bienvenido a DevBlog',
                'content': '''Este es mi primer post en DevBlog, una aplicacion creada para aprender DevOps y CI/CD.
    ...
    def get_all_posts(self) -> List[BlogPost]:
        """
        Obtiene todos los posts ordenados por fecha (mas reciente primero)
        Returns:
    ...
    def search_posts(self, query: str) -> List[BlogPost]:
        """Busca posts que contengan la query en titulo o contenido
        Args:
            query: Termino de busqueda
        Returns:
            Lista de posts que coinciden con la busqueda
        """
        ...
        results = []
        for post in self._posts:
            #Busca en titulo y contenido (case-insensitive)
            if(query in post.title.lower() or 
               query in post.content.lower()):
                results.append(post)
        
        #Ordena por relevancia (titulo primero, luego por fecha)
        results.sort(key=lambda x: (
            query not in x.title.lower(), #Titulo tiene propiedad
...
#En una aplicacion real, esto seria inyectado como dependencia
